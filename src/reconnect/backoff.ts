...
export const TERMINAL_CODES = new Set<number>([
 ...
 */
export function isTerminalError(statusCode: number | undefined): boolean {
...
): "terminal" | "transient" | "unknown" {
  if (statusCode === undefined) return "unknown";
  if (TERMINAL_CODES.has(statusCode)) return "terminal";
  return "transient";
}
