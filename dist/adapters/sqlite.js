...
export class SQLiteAdapter {
    ...
    /* ─── Message store ─── */
    getMessage(sessionName, messageId) {
        const row = this.db
            .prepare(`SELECT key_id AS keyId, message, timestamp
         FROM messages WHERE session_name = ? AND key_id = ?`)
            .get(sessionName, messageId);
        if (!row)
            return Promise.resolve(null);
        return Promise.resolve({
            keyId: row.keyId,
    ...
    putMessage(sessionName, msg) {
        this.db
            .prepare(`INSERT INTO messages (session_name, key_id, message, timestamp)
         VALUES (?, ?, ?, ?)
         ON CONFLICT(session_name, key_id) DO UPDATE SET
           message = excluded.message,
           timestamp = excluded.timestamp`)
            .run(sessionName, msg.keyId, JSON.stringify(msg.message), msg.timestamp ?? null);
        return Promise.resolve();
    }
    ...
    upsertLidMapping(lid, pn) {
        this.db
            .prepare(`INSERT INTO lid_mappings (lid, pn) VALUES (?, ?)
    ...
    listContacts() {
        const rows = this.db
            .prepare(`SELECT id, name, push_name AS pushName, number,
    ...
    upsertContact(contact) {
        this.db
            .prepare(`INSERT INTO contacts (id, name, push_name, number, is_my_contact, is_blocked)
         VALUES (?, ?, ?, ?, ?, ?)
         ON CONFLICT(id) DO UPDATE SET
           ...
           is_blocked = excluded.is_blocked`)
            .run(contact.id, contact.name ?? null, contact.pushName ?? null, contact.number ?? null, contact.isMyContact ? 1 : 0, contact.isBlocked ? 1 : 0);
        return Promise.resolve();
    }
    /* ─── Chats ─── */
    listChats() {
        const rows = this.db
            .prepare(`SELECT id, name, phone_jid AS phoneJid, unread_count AS unreadCount,
    ...
    upsertChat(chat) {
        this.db
            .prepare(`INSERT INTO chats (id, name, phone_jid, unread_count, last_message_timestamp)
         VALUES (?, ?, ?, ?, ?)
         ON CONFLICT(id) DO UPDATE SET
           name = excluded.name,
           ...
           last_message_timestamp = excluded.last_message_timestamp`)
            .run(chat.id, chat.name ?? null, chat.phoneJid ?? null, chat.unreadCount ?? 0, chat.lastMessageTimestamp ?? null);
        return Promise.resolve();
    }
}
