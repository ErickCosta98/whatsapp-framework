...
export class PostgresAdapter {
    ...
    async getMessage(sessionName, messageId) {
        const result = await this.pool.query(`SELECT key_id AS "keyId", message, timestamp
       FROM messages WHERE session_name = $1 AND key_id = $2`, [sessionName, messageId]);
        if (result.rows.length === 0)
            return null;
        const r = result.rows[0];
        return {
    ...
    async putMessage(sessionName, msg) {
        await this.pool.query(`INSERT INTO messages (session_name, key_id, message, timestamp)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (session_name, key_id) DO UPDATE SET
         message = EXCLUDED.message,
         timestamp = EXCLUDED.timestamp`, [sessionName, msg.keyId, JSON.stringify(msg.message), msg.timestamp ?? null]);
    }
    ...
    async getLidMapping(lid) {
        const result = await this.pool.query(`SELECT pn FROM lid_mappings WHERE lid = $1`, [lid]);
        if (result.rows.length === 0)
            return null;
        return result.rows[0].pn ?? null;
    }
    async upsertLidMapping(lid, pn) {
        await this.pool.query(`INSERT INTO lid_mappings (lid, pn) VALUES ($1, $2)
       ON CONFLICT (lid) DO UPDATE SET pn = EXCLUDED.pn`, [lid, pn]);
    }
    /* ─── Contacts ─── */
    async listContacts() {
        const result = await this.pool.query(`SELECT id, name, push_name AS "pushName", number,
    ...
    async upsertContact(contact) {
        await this.pool.query(`INSERT INTO contacts (id, name, push_name, number, is_my_contact, is_blocked)
       VALUES ($1, $2, $3, $4, $5, $6)
    ...
    /* ─── Chats ─── */
    async listChats() {
        const result = await this.pool.query(`SELECT id, name, phone_jid AS "phoneJid", unread_count AS "unreadCount",
    ...
    async upsertChat(chat) {
        await this.pool.query(`INSERT INTO chats (id, name, phone_jid, unread_count, last_message_timestamp)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (id) DO UPDATE SET
         name = EXCLUDED.name,
