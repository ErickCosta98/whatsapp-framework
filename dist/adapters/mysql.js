...
export class MySQLAdapter {
    ...
    /* ─── Message store ─── */
    async getMessage(sessionName, messageId) {
        const [rows] = await this.pool.execute(`SELECT key_id AS keyId, message, timestamp
       FROM messages WHERE session_name = ? AND key_id = ?`, [sessionName, messageId]);
        if (rows.length === 0)
            return null;
        const r = rows[0];
        return {
    ...
    async putMessage(sessionName, msg) {
        await this.pool.execute(`INSERT INTO messages (session_name, key_id, message, timestamp)
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         message = VALUES(message),
         timestamp = VALUES(timestamp)`, [sessionName, msg.keyId, JSON.stringify(msg.message), msg.timestamp ?? null]);
    }
    async clearSessionMessages(sessionName) {
        await this.pool.execute(`DELETE FROM messages WHERE session_name = ?`, [sessionName]);
    }
    /* ─── LID mappings ─── */
    async getLidMapping(lid) {
        const [rows] = await this.pool.execute(`SELECT pn FROM lid_mappings WHERE lid = ?`, [lid]);
        if (rows.length === 0)
            return null;
        return rows[0].pn ?? null;
    }
    async upsertLidMapping(lid, pn) {
        await this.pool.execute(`INSERT INTO lid_mappings (lid, pn) VALUES (?, ?)
       ON DUPLICATE KEY UPDATE pn = VALUES(pn)`, [lid, pn]);
    }
    /* ─── Contacts ─── */
    async listContacts() {
        const [rows] = await this.pool.execute(`SELECT id, name, push_name AS pushName, number,
    ...
    async upsertContact(contact) {
        await this.pool.execute(`INSERT INTO contacts (id, name, push_name, number, is_my_contact, is_blocked)
       VALUES (?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         name = VALUES(name),
         push_name = VALUES(push_name),
    ...
    /* ─── Chats ─── */
    async listChats() {
        const [rows] = await this.pool.execute(`SELECT id, name, phone_jid AS phoneJid, unread_count AS unreadCount,
    ...
    async upsertChat(chat) {
        await this.pool.execute(`INSERT INTO chats (id, name, phone_jid, unread_count, last_message_timestamp)
       VALUES (?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         name = VALUES(name),
